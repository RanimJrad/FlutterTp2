...
class _NumberToWordsScreenState extends State<NumberToWordsScreen> {
  ...
  void _convertToArabic() {
    ...
    if (number.isNotEmpty) {
      ...
      final int dinar = intNumber.toInt(); // Use full number for dinars
      final int milli =
          ((intNumber - dinar) * 1000).round(); // Get the milli from the input

      setState(() {
        // Constructing the Arabic representation
        String dinarText =
            Tafqeet.convert(dinar.toString()); // Convert to String
        String milliText = milli > 0
            ? Tafqeet.convert(milli.toString())
            : ''; // Convert to String only if milli is greater than 0

        // Combine the results in the correct format
        if (milli > 0) {
  ...
  void _convertToEnglish() {
    ...
    if (number != null) {
      ...
      setState(() {
        // Only include "cents" if fractionPart is greater than zero
        if (fractionPart > 0) {
          _convertedText =
  ...
  String _numberToWordsEnglish(int number) {
    ...
    return number.toString(); // For larger numbers, you can extend this logic
  }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Container(
              color: Colors.purpleAccent, // Set the background color here
